...
const superMoreUtil = {
  ...
  open(params = {}, callback) {
    ...
    const { pluginData, routerConfigUrl, isColmo, bluetoothEnter } = params
    this.getDeviceInfo().then(resData => {
      const { deviceId } = resData.result
      const storageKey = MENU_CONIFG_KEY + deviceId
      ...
      storage.setItem(storageKey, storageData, () => {
        ...
        bridgeModule.commandInterface(
          ...
          resData => {
            if (resData) {
              const pluginMenuPath = `${JSON.parse(resData).path}/weex.js`
  ...
  jumpPluginMenu(path, isColmo) {
    ...
    const options = isColmo ? { bgColor: '#000000' } : {}
    options.url = path
  ...
  reset() {
    ...
    this.getDeviceInfo().then(resData => {
      const { deviceId } = resData.result
      const storageKey = MENU_CACHE_KEY + deviceId
      // 清除菜单使用缓存的标识
