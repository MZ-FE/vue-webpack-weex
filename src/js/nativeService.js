...
let isRemote = weex.config.bundleUrl.indexOf('http') > -1 ? true : false
...
export default {
  ...
  goTo(path, options, params) {
    ...
    if (this.isDummy != true && !isRemote) {
      ...
      //PC网页调试跳转
      location.href =
        location.origin + location.pathname + '?path=' + path.replace('?', '&')
    }
  },
  runGo(url, options) {
  ...
  getPath(callBack) {
    if (this.isDummy != true && !isRemote) {
      bridgeModule.getWeexPath(resData => {
        ...
        let weexPath = jsonData.weexPath
        callBack(weexPath)
      })
    } else if (platform != 'Web') {
      //手机远程weex页面调试
  ...
  alert(message, callback, okTitle) {
    let callbackFunc = callback || function (value) {}
  ...
  /* ^6.1.0
        设置状态栏
        根据业务设置手机状态栏的背景以及字体颜色。以适应不同颜色背景的页面
        params:{
  ...
  /* ^6.3 (iOS专用)
        获取App当前是否有（美居应用系统权限设置）通知权限, 未开启通知权限的时候弹出弹框
        params:{
        }
    }
   */
  checkCurrentNotificationStatus() {
    let param = Object.assign(
      {},
      {
        operation: 'checkCurrentNotificationStatus',
      }
    )
    return this.commandInterfaceWrapper(param)
  },
  ...
  sendMCloudRequest(
    name,
    params,
    options = { isShowLoading: true, isValidate: true }
  ) {
    return new Promise((resolve, reject) => {
      let self = this
      if (this.isDummy != true) {
        ...
        let resData = this.Mock.getMock(
          self.serviceList[name] ? self.serviceList[name] : name
        )
        if (options.isValidate) {
          //resData.status为5.0判断；resData.errorCode为4.判断
          if (resData.errorCode == 0) {
  ...
  sendCentralCloundRequest(name, params, options = { isShowLoading: true }) {
    return new Promise((resolve, reject) => {
      if (this.isDummy != true) {
        ...
        let resData = this.Mock.getMock(
          this.serviceList[name] ? this.serviceList[name] : name
        )
        resolve(resData)
      }
    })
  ...
  startCmdProcess(name, messageBody, callback, callbackFail) {
    ...
    let finalCallBack = function (resData) {
      if (typeof resData == 'string') {
        resData = JSON.parse(resData)
      }
      if (resData.errorCode != 0) {
    ...
    let finalCallbackFail = function (resData) {
      if (typeof resData == 'string') {
        resData = JSON.parse(resData)
      }
      callbackFail(resData)
  ...
  callbackFunction(commandId, result) {
    ...
    let cbf = this.callbackFunctions[commandId]
    let cbff = this.callbackFailFunctions[commandId]
    if (
      jsonResult.errorCode !== undefined &&
      jsonResult.errMessage == 'TimeOut'
    ) {
  ...
  /*
   * ^6.0.0 解绑设备，若是解绑成功，则发出解绑成功的页面刷新事件
   * @params: {
   *  applianceCode: xxxxx, // 必传参数，设备id
   *  isOtherEquipment: 0 , // 非必须参数，是否为第三方设备 1 是 , 0 否，默认为0
   * }
   */
  unbindDevice(params) {
    let param = Object.assign(params, {
  ...
  /*
   * ^6.5.0 读取weex模块路径（仅安卓）
   * @params: {
   *  folder: xxxxx, // 必传参数，weex包的文件夹名称，如community
   * }
   */
  getWeexFunctionPath(params) {
    let param = Object.assign(params, {
  ...
  disconnectBlueMeshConnection(params = {}) {
    return this.blueToothMeshModuleWrapper(
      'disconnectBlueMeshConnection',
      params
    )
  },
  //获取当前家庭的所有Mesh设备的信息
  ...
  deleteBlueMeshModelSubscription(params = {}) {
    return this.blueToothModuleWrapper(
      'deleteBlueMeshModelSubscription',
      params
    )
  },
  //**********蓝牙接口***************END
